...
func _process(delta: float) -> void:
	...
	if Input.is_action_pressed("click") and Global.is_dragging:
		...
		if $Area2D.get_overlapping_areas().any(func(area): return area.is_in_group("counter")):
			Global.increment_drink_value("vol")  # Jager increases volume
			Global.increment_drink_value("sweetness")  # Jager is sweet
			Global.increment_drink_value("herbalness")  # Jager has herbal notes
